...
const Log: FunctionComponent<LogProps> = (
  ...
  let message = isError && log.args.length === 0 ? log.text : log.args.map(a => typeof a === 'object' ? JSON.stringify(a) : a).join(' ');
          ...
          {isError && <>
            <table className='small'>
              ...
              {log.stackTrace.map((trace, idx) => (
                <tr key={idx}>
                  <td>{trace.lineNumber}{trace.columnNumber && ':'}{trace.columnNumber}</td>
...
export const Context: FunctionComponent<ContextProps> = observer((
  {
    context,
  },
) => {
  ...
  let events = [
    ...context.commands.filter(typeFilter),
    ...context.logs,
  ];
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <table
          className={'snapshots mx-2 mb-2'}
          cellSpacing={0}
          cellPadding={0}
        >
          <tbody>
          {events.map((event, idx) => event.type === 'command' ? <Command
            key={idx}
            index={commandIndex++}
            command={event}
          /> : <Log
